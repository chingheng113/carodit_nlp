...
from transformers import AdamW, XLNetTokenizer, XLNetForSequenceClassification, XLNetModel, XLNetLMHeadModel, XLNetConfig
...
def generate_predictions(model, df, num_labels, device="cpu", batch_size=32):
    ...
    for i in range(num_iter):
        ...
        with torch.no_grad():
            ...
            m = torch.nn.Sigmoid()
            logits = torch.squeeze(m(outputs))
...
def model_testing(trained_model, test_data, label_cols, exin, round_n):
    ...
    if exin == 'internal':
        ...
        with open(os.path.join('results_binary', exin, 'predict_result_'+round_n+'.pickle'), 'wb') as file_pi:
            pickle.dump(test_data, file_pi)
...
def model_training(train_data, label_cols, round_n):
    ...
    model_save_path = output_model_file = os.path.join('models_binary', 'round_'+round_n)
    ...
    # if train Model From Previous Checkpoint
...
if __name__ == '__main__':
    ...
    if ex_in == 'internal':
        ...
        model, start_epoch, lowest_eval_loss, train_loss_hist, valid_loss_hist = load_model(os.path.join('models_binary', 'round_'+round_num))
        model_testing(model, test_dataset, label_names, ex_in, round_num)
    else:
