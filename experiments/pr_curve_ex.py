...
for inx, col in enumerate(cols):
    ...
    for n in range(10):
        # xlnet
        xlnet_read_path = os.path.join('xlnet', 'results', 'external', 'predict_result_'+ str(n) +'.pickle')
    ...
    # rb_precision, rb_recall, rb_thresholds = precision_recall_curve(true_label, predict_label)
    # rb_pr_auc = auc(rb_recall, rb_precision)
    # rb_auc = rb_pr_auc
    ...
    # ax.plot(rb_recall, rb_precision, label=('Rule-based (area = {0:0.2f})'''.format(rb_auc)))
