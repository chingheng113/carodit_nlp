...
def read_internal_data_binary(n):
...
def read_external_data_binary(n):
...
def model_training(config, train, label_cols, round_n):
    ...
    history = model.fit(t2s_train_pad, y_train,
                        ...
                        callbacks=[
                            ...
                            ModelCheckpoint(os.path.join('models_binary', model.name+'.h5'),
                                            save_best_only=True, verbose=1)
                        ])
    end = time.time()
    elapse = end - start
    # History
    with open(os.path.join('results_binary', 'internal', 'round_'+round_n, 'history_'+round_n+'.pickle'), 'wb') as file_pi:
        pickle.dump(history.history, file_pi)

    with open(os.path.join('results_binary', 'internal', 'round_'+round_n, 'elapse_time.pickle'), 'wb') as file_pi:
...
def model_testing(testing, label_cols, model, tokenizer, max_len, round_n, ex_in):
    ...
    if ex_in == 'internal':
        testing.to_csv(os.path.join('results_binary', 'internal', 'round_'+round_n, 'predict_result_'+round_n+'.csv'), index=False)
    else:
        testing.to_csv(os.path.join('results_binary', 'external', 'predict_result_'+round_n+'.csv'), index=False)
    print('testing done')
...
if __name__ == '__main__':
    ...
    in_train_data, in_test_data, in_label_cols = read_internal_data_binary(round_nm)
    ...
    ex_test_data, ex_label_cols = read_external_data_binary(round_nm)
